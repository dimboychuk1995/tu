...
(function(b){
    ...
    b.extend(b.jgrid,{
        ...
        format:function(f){
            ...
            if(f===undefined)f="";
            return f.replace(/\{(\d+)\}/g,function(i,
                c){
                return j[c]
                })
            },
        ...
        parseDate:function(f,j){
            ...
            if(j&&j!==null&&j!==undefined){
                ...
                j=j.split(/[\\\/:_;.\t\T\s-]/);
                f=f.split(/[\\\/:_;.\t\T\s-]/);
                var m=b.jgrid.formatter.date.monthNames,a=b.jgrid.formatter.date.AmPm,r=function(u,B){
                    if(u===0){
                        if(B==12)B=0
                            }else if(B!=12)B+=12;
                    return B
...
from:function(f){
    return new (function(j,i){
        ...
        if(typeof j=="object"&&j.push){
            if(j.length>0)M=typeof j[0]!="object"?false:true
    ...
    this._doSort=function(n,l){
        ...
        if(l==J.length-1)return c._getOrder(n,p,A,t,H);
        l++;
        n=c._getGroup(n,p,A,t,H);
        p=[];
        for(A=0;A<n.length;A++){
            t=c._doSort(n[A].items,l);
            for(H=0;H<t.length;H++)p.push(t[H])
                }
                return p
        };
    ...
    this._getOrder=function(n,l,p,A,t){
        ...
        fa=A=="float"||A=="number"||A=="currency"||A=="numeric"?function(P){
    ...
    this._compareValues=function(n,l,p,A,t){
        ...
        switch(t.stype===undefined?"text":t.stype){
            ...
            case "date":case "datetime":
                p=String(b.jgrid.parseDate(t.newfmt||"Y-m-d",p).getTime());
                H='jQuery.jgrid.parseDate("'+
                t.srcfmt+'",'+H+").getTime()";
                break;
            default:
                H=c._getStr(H);
    ...
    this.startsWith=function(n,l){
        ...
        if(M)c._append(c._getStr("this."+n)+".substr(0,"+p+") == "+c._getStr('"'+c._toStr(l)+'"'));
        else{
    ...
    this.endsWith=function(n,l){
        ...
        l===null?n:l;
        p=m?b.trim(p.toString()).length:p.toString().length;
        M?c._append(c._getStr("this."+n)+".substr("+c._getStr("this."+n)+".length-"+p+","+p+') == "'+c._toStr(l)+'"'):c._append(c._getStr("this")+".substr("+c._getStr("this")+'.length-"'+c._toStr(n)+'".length,"'+c._toStr(n)+'".length) == "'+c._toStr(n)+'"');
    ...
    this.contains=function(n,l){
        M?c._append(c._getStr("this."+n)+'.indexOf("'+c._toStr(l)+'",0) > -1'):c._append(c._getStr("this")+'.indexOf("'+
            c._toStr(n)+'",0) > -1');
    ...
    this.orderBy=function(n,l,p,A){
        l=l===undefined||l===null?"a":b.trim(l.toString().toLowerCase());
        if(p===null||p===undefined)p="text";
        if(A===null||A===undefined)A="Y-m-d";
        if(l=="desc"||l=="descending")l="d";
        if(l=="asc"||l=="ascending")l="a";
        J.push({
            by:n,
            dir:l,
            type:p,
    ...
    if(d){
        ...
        if(h){
            ...
            if(w<d&&x<=0&&(c.lastpage===
                undefined||parseInt((w+o+h-1)/h,10)<=c.lastpage)){
...
selectionPreserver:function(d){
    var g=d.p,h=g.selrow,q=g.selarrrow?b.makeArray(g.selarrrow):null,o=d.grid.bDiv.scrollLeft,x=g.gridComplete;
    g.gridComplete=function(){
        g.selrow=null;
        g.selarrrow=[];
        if(g.multiselect&&q&&q.length>0)for(var w=0;w<q.length;w++)q[w]!=h&&b(d).jqGrid("setSelection",q[w],
            false);
...
if(this.tagName!="TABLE")alert("Element is not a table");
    ...
    if(this.p.colNames.length!==this.p.colModel.length)alert(b.jgrid.errors.model);
        ...
        var r=b("<div class='ui-jqgrid-view'></div>"),u,B=b.browser.msie?true:false,
        G=b.browser.safari?true:false;
        a=this;
        ...
        var J=function(d,g){
            ...
            var q=a.p.colModel[d],o=q.align,x='style="',
            w=q.classes,C=q.name;
            if(o)x+="text-align:"+o+";";
            ...
            if(typeof h.formatter!=="undefined"){
                ...
                g=b.isFunction(h.formatter)?
                h.formatter.call(a,g,d,q,o):b.fmatter?b.fn.fmatter(h.formatter,g,d,q,o):n(g)
                }else g=n(g);
            return g
            ...
            return'<td role="gridcell" '+M(g,h,"")+">"+d+"</td>"
            },t=function(d,g,h,q){
            ...
            for(o=0;o<a.p.colModel.length;o++){
                ...
                if(g.name!=="cb"&&g.name!=="subgrid"&&g.name!=="rn"){
                    h[q]=d=="local"?g.name:d=="xml"?g.xmlmap||g.name:g.jsonmap||g.name;
                    q++
                }
            }
            return h
        ...
        if(!g||!g.length)g=b.map(a.p.colModel,function(h,q){
            return q
            });
    ...
    g,h,q,o){
    var x=new Date,w=a.p.datatype!="local"&&a.p.loadonce||a.p.datatype=="xmlstring",C,D=a.p.datatype=="local"?"local":"xml";
    if(w){
        a.p.data=[];
        a.p._index={};
    ...
    if(b.isXMLDoc(d)){
        ...
        var s,v=0,y,E,I=0,F=0,K=0,z,U=[],W,S={},N,L,X=[],oa=a.p.altRows===true?" "+a.p.altclass:"";
        a.p.xmlReader.repeatitems||(U=H(D));
        z=a.p.keyIndex===false?a.p.xmlReader.id:a.p.keyIndex;
        if(U.length>0&&!isNaN(z)){
            if(a.p.remapColumns&&
                a.p.remapColumns.length)z=b.inArray(z,a.p.remapColumns);
            z=U[z]
            }
            D=(z+"").indexOf("[")===-1?U.length?function(da,Z){
        ...
        var ga=d.length,$=0;
        if(d&&ga){
            var ha=parseInt(a.p.rowNum,10),ra=a.p.scroll?(parseInt(a.p.page,10)-1)*ha+1:1;
            if(o)ha*=o+1;
            ...
            for(;$<ga;){
                ...
                if(a.p.grouping){
                    ...
                    E=[];
                    for(var ya=0;ya<s;ya++)E.push(S[a.p.groupingView.groupField[ya]]);
                ...
                if(a.p.subGrid===true)try{
                    b(a).jqGrid("addSubGrid",a.rows[a.rows.length-1],I+K)
                    }catch(Ja){}
...
if(w){
    ...
    if(d){
        ...
        if(o)oa*=o+1;
        var $=b.isFunction(a.p.afterInsertRow),
        ha={},ra="";
        ...
        for(;d<W;){
            ...
            if(a.p.rownumbers===true){
                ...
                if(a.p.subGrid){
                E.push(b(a).jqGrid("addSubGridCell",
                    K+U,d+h));
                z=1
                }
                if(g.repeatitems){
                if(g.cell)o=b.jgrid.getAccessor(o,g.cell);
                F||(F=T(K+z+U))
                }
            ...
            if(a.p.grouping){
                ...
                y=[];
                for(var ia=0;ia<v;ia++)y.push(N[a.p.groupingView.groupField[ia]]);
                ...
                }else b("#"+a.p.id+" tbody:first").append(E.join(""));
            if(a.p.subGrid===true)try{
                b(a).jqGrid("addSubGrid",a.rows[a.rows.length-1],K+U)
                }catch(Ba){}
        ...
        if(a.p.gridview===true)a.p.grouping?b(a).jqGrid("groupingRender",ha,a.p.colModel.length):b("#"+a.p.id+" tbody:first").append(E.join(""));
    a.p.totaltime=new Date-x;
        ...
        b.each(a.p.colModel,function(){
            ...
            if(x=="date"||x=="datetime"){
                if(this.formatter&&typeof this.formatter==="string"&&this.formatter=="date"){
                    o=this.formatoptions&&this.formatoptions.srcformat?this.formatoptions.srcformat:b.jgrid.formatter.date.srcformat;
                    w=this.formatoptions&&this.formatoptions.newformat?this.formatoptions.newformat:
...
if(a.p.search===true){
    ...
    s=D[a.p.postData.searchOper](s)(a.p.postData.searchField,a.p.postData.searchString,h[a.p.postData.searchField])
    }catch(K){}
}
if(a.p.grouping){
    s.orderBy(C.groupField[0],C.groupOrder[0],q[0].stype,q[0].srcfmt);
    C.groupDataSorted=true
    }
    if(d&&a.p.sortorder&&g)a.p.sortorder.toUpperCase()==
    "DESC"?s.orderBy(a.p.sortname,"d",h[d].stype,h[d].srcfmt):s.orderBy(a.p.sortname,"a",h[d].stype,h[d].srcfmt);
D=s.select();
        ...
        if(b.isFunction(a.p.datatype))a.p.datatype.call(a,a.p.postData,"load_"+a.p.id);
            ...
            switch(q){
                case "json":case "jsonp":case "xml":case "script":
                    ...
                    success:function(s){
                        q==="xml"?fa(s,a.grid.bDiv,D,d>1,C):ca(s,a.grid.bDiv,D,d>1,C);
                        x&&x.call(a,s);
...
u=function(d,g){
    var h="",q="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>",
    o="",x,w,C,D,s=function(v){
        var y;
    ...
    b("#"+d).append("<div id='"+x+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='"+
        w+"' align='left'></td><td id='"+C+"' align='center' style='white-space:pre;'></td><td id='"+D+"' align='right'></td></tr></tbody></table></div>").attr("dir","ltr");
    if(a.p.rowList.length>0){
        o="<td dir='"+m+"'>";
        o+="<select class='ui-pg-selbox' role='listbox'>";
        for(w=0;w<a.p.rowList.length;w++)o+='<option role="option" value="'+a.p.rowList[w]+'"'+(a.p.rowNum==a.p.rowList[w]?' selected="selected"':"")+">"+a.p.rowList[w]+"</option>";
        o+="</select></td>"
        }
        if(m=="rtl")q+=o;
    if(a.p.pginput===true)h="<td dir='"+
        m+"'>"+b.jgrid.format(a.p.pgtext||"","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1'></span>")+"</td>";
    ...
    if(m=="ltr")q+=o;
    q+="</tr></tbody></table>";
    a.p.viewrecords===true&&b("td#"+d+"_"+a.p.recordpos,"#"+x).append("<div dir='"+m+"' style='text-align:"+a.p.recordpos+
...
var Ca=function(d,g,h,q){
    if(a.p.colModel[g].sortable)if(!(a.p.savedRow.length>0)){
        if(!h){
            ...
            if(q)if(a.p.lastsort==g&&a.p.sortorder==q&&!h)return;else a.p.sortorder=q;
    ...
    for(g=h[0]=h[1]=h[2]=0;g<=d;g++)if(a.p.colModel[g].hidden===false)h[0]+=a.p.colModel[g].width+q;if(a.p.direction=="rtl")h[0]=a.p.width-h[0];
...
a.p.keyIndex=false;
for(k=0;k<a.p.colModel.length;k++)if(a.p.colModel[k].key===true){
    a.p.keyIndex=k;
    break
}
...
if(a.p.grouping===true){
    ...
    this.p.colModel.unshift({
        name:"cb",
        width:G?a.p.multiselectWidth+a.p.cellLayout:a.p.multiselectWidth,
        sortable:false,
        resizable:false,
        hidedlg:true,
        search:false,
...
na="<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+na+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+m+"'></span>";
na+="<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+ma+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+m+"'></span></span>";
for(k=0;k<this.p.colNames.length;k++){
    ma=a.p.headertitles?' title="'+b.jgrid.stripHtml(a.p.colNames[k])+
    '"':"";
    ba+="<th id='"+a.p.id+"_"+a.p.colModel[k].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+m+"'"+ma+">";
...
if(isNaN(a.p.width))a.p.width=e.width=d;else e.width=a.p.width;
    ...
    b.each(a.p.colModel,function(E){
        if(this.hidden===false&&!this.fixed){
            this.width=
            x=Math.round(C*this.width/(a.p.tblwidth-D));
            d+=x;
            q=E
            }
        });
y=0;
if(w){
    if(e.width-s-(d+g*h)!==o)y=e.width-s-(d+g*h)-o
    ...
    a.p.colModel[q].width-=a.p.tblwidth-parseInt(a.p.width,10);
...
var va="";
if(a.p.footerrow)va+="<table role='grid' style='width:"+a.p.tblwidth+"px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-"+m+"'>";
r=b("tr:first",ba);
var wa="<tr class='jqgfirstrow' role='row' style='height:auto'>";
a.p.disableClick=false;
b("th",r).each(function(d){
    ...
    if(a.p.colModel[d].resizable){
        ...
        b(qa).html("&#160;").addClass("ui-jqgrid-resize ui-jqgrid-resize-"+m);
...
k=b("<div class='ui-jqgrid-hbox"+(m=="rtl"?"-rtl":"")+"'></div>");
ba=null;
...
b(a).before(e.hDiv).click(function(d){
    ...
    if(V.tagName=="A"||(V.tagName=="INPUT"||
        V.tagName=="TEXTAREA"||V.tagName=="OPTION"||V.tagName=="SELECT")&&!g)return this;
    if(h===true){
    ...
    if(a.p.multiselect&&
        a.p.multiboxonly)if(!g){
        b(a.p.selarrrow).each(function(o,x){
    ...
    if(g&&g.page){
        ...
        if(d<1)d=1;
        a.p.page=d;
        a.grid.bDiv.scrollTop=a.grid.prevRowHeight?(d-1)*a.grid.prevRowHeight*a.p.rowNum:
        0
        }
...
b(e.bDiv).append(b('<div style="position:relative;'+(B&&b.browser.version<8?"height:0.01%;":"")+'"></div>').append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({
...
var Aa=a.p.hidegrid===true?b("<a role='link' href='javascript:void(0)'/>").addClass("ui-jqgrid-titlebar-close HeaderButton").hover(function(){
    ...
    }).append("<span class='ui-icon ui-icon-circle-triangle-n'></span>").css(m==
    "rtl"?"left":"right","0px"):"";
b(e.cDiv).append(Aa).append("<span class='ui-jqgrid-title"+(m=="rtl"?"-rtl":"")+"'>"+a.p.caption+"</span>").addClass("ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix");
b(e.cDiv).insertBefore(e.hDiv);
if(a.p.toolbar[0]){
    e.uDiv=document.createElement("div");
    if(a.p.toolbar[1]=="top")b(e.uDiv).insertBefore(e.hDiv);else a.p.toolbar[1]=="bottom"&&b(e.uDiv).insertAfter(e.hDiv);
    if(a.p.toolbar[1]=="both"){
        e.ubDiv=document.createElement("div");
        b(e.uDiv).insertBefore(e.hDiv).addClass("ui-userdata ui-state-default").attr("id",
            "t_"+this.id);
        ...
        b(".ui-jqgrid-titlebar-close",e.cDiv).click(function(d){
            ...
            if(a.p.gridstate==
                ...
                if(a.p.toolbar[0]===true){
                    a.p.toolbar[1]=="both"&&b(e.ubDiv).slideUp("fast");
                ...
                b(".ui-resizable-handle","#gbox_"+a.p.id).hide();
                ...
                if(a.p.toolbar[0]===true){
                    a.p.toolbar[1]=="both"&&b(e.ubDiv).slideDown("fast");
...
setSelection:function(f,j){
    return this.each(function(){
        function i(m){
            ...
            if(u+m>=a+r)b(c.grid.bDiv)[0].scrollTop=u-(a+r)+m+r;
            else if(u<a+r)if(u<r)b(c.grid.bDiv)[0].scrollTop=
                u
                }
                var c=this,e,k;
        if(f!==undefined){
        ...
        if(typeof f=="undefined"){
            ...
            if(b(r).hasClass("jqgrow")){
                b("td",r).each(function(u){
                    ...
                    if(a!=="cb"&&a!=="subgrid"&&a!=="rn")if(m.p.treeGrid===true&&a==m.p.ExpandColumn)j[a]=b.jgrid.htmlDecode(b("span:first",this).html());else try{
                        j[a]=b.unformat(this,{
                            rowId:r.id,
        ...
        if(j)try{
            b(this.p.colModel).each(function(J){
                ...
                if(j[c]!==undefined){
                    ...
                    m.p.treeGrid===true&&c==m.p.ExpandColumn?b("td:eq("+J+") > span:first",r).html(a).attr(k):b("td:eq("+J+")",r).html(a).attr(k)
                    }
                });
        if(m.p.datatype=="local"){
            var G=m.p._index[f];
            if(typeof G!="undefined")m.p.data[G]=b.extend(true,m.p.data[G],B);
            B=null
            }
        }catch(Q){
        e=false
        }
        if(e)if(u==="string")b(r).addClass(i);else u==="object"&&b(r).css(i)
        });
return e
},
addRowData:function(f,j,i,c){
    i||(i="last");
    var e=false,k,m,a,r,u,B,G,Q,
    J="",M,n,l,p,A;
    if(j){
        if(b.isArray(j)){
            M=true;
            ...
            l=t.p.altclass;
        for(var T=0,Y="",O={},fa=b.isFunction(t.p.afterInsertRow)?true:false;T<H;){
            p=j[T];
            m="";
            if(M){
                try{
                ...
                if(u){
                J=t.formatCol(0,1,"");
                m+='<td role="gridcell" aria-describedby="'+t.p.id+'_rn" class="ui-state-default jqgrid-rownum" '+J+">0</td>"
                }
                if(a){
                Q='<input role="checkbox" type="checkbox" id="jqg_'+t.p.id+"_"+f+'" class="cbox"/>';
                J=t.formatCol(u,1,"");
                m+='<td role="gridcell" aria-describedby="'+t.p.id+'_cb" '+J+">"+Q+"</td>"
                }
                if(r)m+=b(t).jqGrid("addSubGridCell",a+u,1);
            for(G=a+r+u;G<t.p.colModel.length;G++){
                ...
                J=t.formatCol(G,1,Q);
                m+='<td role="gridcell" aria-describedby="'+t.p.id+"_"+k+'" '+J+">"+Q+"</td>"
                }
                m='<tr id="'+f+'" role="row" class="ui-widget-content jqgrow ui-row-'+t.p.direction+" "+Y+'">'+m+"</tr>";
            if(t.p.subGrid===true){
                m=b(m)[0];
                b(t).jqGrid("addSubGrid",m,a+u)
                }
                if(t.rows.length===0)b("table:first",t.grid.bDiv).append(m);else switch(i){
                case "last":
                    b(t.rows[t.rows.length-
                    1]).after(m);
                    break;
                case "first":
                    b(t.rows[0]).after(m);
                    break;
                case "after":
                    if(B=t.rows.namedItem(c))b(t.rows[B.rowIndex+1]).hasClass("ui-subgrid")?b(t.rows[B.rowIndex+1]).after(m):b(B).after(m);
                    break;
                case "before":
        ...
        if(t.p.altRows===true&&!M)if(i=="last")(t.rows.length-1)%2==1&&b(t.rows[t.rows.length-
        1]).addClass(l);else b(t.rows).each(function(P){
...
footerData:function(f,j,i){
    function c(r){
        for(var u in r)if(r.hasOwnProperty(u))return false;return true
        }
        var e,k=false,m={},a;
    ...
    this.each(function(){
        ...
        b(this.p.colModel).each(function(B){
            ...
            if(f=="set"){
                ...
                }else if(f=="get")m[e]=b("tr.footrow td:eq("+B+")",r.grid.sDiv).html()
            })
    });
return f=="get"?m:k
},
        ...
        if(this.grid){
            ...
            if(!isNaN(f)){
                ...
                if(a!==0){
                    ...
                    b.each(i.p.colModel,function(l){
                        if(this.hidden===false&&!this.fixed){
                            c=Math.round(B*this.width/(i.p.tblwidth-Q));
                            if(!(c<0)){
                                this.width=c;
                                e+=c;
            ...
            M=0;
            if(r){
                if(f-G-(e+k*a)!==u)M=f-G-(e+k*a)-u
                    }else if(Math.abs(f-G-(e+k*a))!==1)M=f-G-(e+k*a);
            i.p.colModel[m].width+=
            M;
...
setCell:function(f,j,i,c,e,k){
    return this.each(function(){
        var m=this,a=-1,r,u;
        if(m.grid){
            if(isNaN(j))b(m.p.colModel).each(function(G){
                if(this.name==j){
                    a=G;
                    return false
                    }
                });else a=parseInt(j,10);
        if(a>=0)if(r=m.rows.namedItem(f)){
            var B=
            b("td:eq("+a+")",r);
            if(i!==""||k===true){
                ...
                m.p.treeGrid&&b(".tree-wrap",b(B)).length>0?b("span",b(B)).html(r).attr(u):b(B).html(r).attr(u);
                if(m.p.datatype=="local"){
                    r=m.p.colModel[a];
                    i=r.formatter&&typeof r.formatter==="string"&&r.formatter=="date"?b.unformat.date(i,r):i;
                    u=m.p._index[f];
                    if(typeof u!="undefined")m.p.data[u][r.name]=i
                        }
                    }
...
getCol:function(f,j,i){
    ...
    this.each(function(){
        ...
        this,a=-1;
        if(m.grid){
            if(isNaN(f))b(m.p.colModel).each(function(G){
                if(this.name===f){
                    a=G;
                    return false
                    }
...
DateFormat:function(a,b,d,e){
    var h=function(m,r){
        m=String(m);
        for(r=parseInt(r,10)||2;m.length<
            r;)m="0"+m;
        return m
        },g={
        m:1,
    ...
    if(b.constructor===Number)f=new Date(b);
    else if(b.constructor===Date)f=b;
    else{
        b=b.split(/[\\\/:_;.,\t\T\s-]/);
        a=a.split(/[\\\/:_;.,\t\T\s-]/);
        i=0;
    ...
    var l=f.getSeconds(),o=f.getMilliseconds(),n=f.getDay(),p=f.getFullYear(),q=(n+6)%7+1,s=(new Date(p,i-1,b)-new Date(p,0,1))/864E5,t={
        d:h(b),
        D:k.i18n.dayNames[n],
        j:b,
        ...
        e:"?",
        I:"?",
        O:(j>0?"-":"+")+h(Math.floor(Math.abs(j)/60)*100+Math.abs(j)%60,4),
        P:"?",
        T:(String(f).match(/\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g)||
            [""]).pop().replace(/[^-+\dA-Z]/g,""),
        Z:"?",
        c:"?",
        r:"?",
...
c.fn.fmatter.checkbox=function(a,b){
    ...
    if(c.fmatter.isEmpty(a)||c.fmatter.isUndefined(a))a=c.fn.fmatter.defaultFormat(a,d);
    a+="";
    a=a.toLowerCase();
...
c.fn.fmatter.link=function(a,b){
    ...
    c.fmatter.isUndefined(b.colModel.formatoptions)||(d=c.extend({},d,
        b.colModel.formatoptions));
    if(d.target)e="target="+d.target;
    return c.fmatter.isEmpty(a)?c.fn.fmatter.defaultFormat(a,b):"<a "+e+' href="'+a+'">'+a+"</a>"
    };
...
c.fn.fmatter.showlink=function(a,b){
    ...
    return c.fmatter.isString(a)||c.fmatter.isNumber(a)?"<a "+e+' href="'+d+'">'+a+"</a>":c.fn.fmatter.defaultFormat(a,b)
    };
...
c.fn.fmatter.date=function(a,b,d,e){
    ...
    e=="edit"?c.fn.fmatter.defaultFormat(a,b):c.fmatter.isEmpty(a)?c.fn.fmatter.defaultFormat(a,b):c.fmatter.util.DateFormat(d.srcformat,a,d.newformat,d)
    };
...
c.fn.fmatter.select=function(a,b){
    ...
    if(d){
        ...
        if(h){
            ...
            if(c.fmatter.isString(d))for(var i=
            d.split(";"),j=0,k=0;k<i.length;k++){
            f=i[k].split(":");
            if(f.length>2)f[1]=jQuery.map(f,function(l,o){
                if(o>0)return l
                    }).join(":");
            if(h){
        ...
        }else if(c.fmatter.isObject(d))if(h)e=jQuery.map(g,function(l){
    return d[l]
    });else e[0]=d[a]||""
    }
    a=e.join(", ");
...
c.fn.fmatter.actions=function(a,b){
    ...
    if(a.editbutton){
        ...
        e=e+"<div style='margin-left:8px;'><div title='"+c.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+h+"><span class='ui-icon ui-icon-pencil'></span></div>"
        }
        if(a.delbutton){
        h="onclick=jQuery('#"+b.gid+"').jqGrid('delGridRow','"+d+"');";
        e=e+"<div title='"+c.jgrid.nav.deltitle+"' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' "+h+"><span class='ui-icon ui-icon-trash'></span></div>"
        }
        h=
    "onclick=$.fn.fmatter.rowactions('"+d+"','"+b.gid+"','save',false);";
    e=e+"<div title='"+c.jgrid.edit.bSubmit+"' style='float:left;display:none' class='ui-pg-div ui-inline-save'><span class='ui-icon ui-icon-disk' "+h+"></span></div>";
    h="onclick=$.fn.fmatter.rowactions('"+d+"','"+b.gid+"','cancel',false);";
    return e=e+"<div title='"+c.jgrid.edit.bCancel+"' style='float:left;display:none;margin-left:5px;' class='ui-pg-div ui-inline-cancel'><span class='ui-icon ui-icon-cancel' "+h+"></span></div></div>"
    };
c.unformat=function(a,b,d,e){
    var h,g=b.colModel.formatter,f=b.colModel.formatoptions||{},i=/([\.\*\_\'\(\)\{\}\+\?\\])/g,j=b.colModel.unformat||c.fn.fmatter[g]&&c.fn.fmatter[g].unformat;
    if(typeof j!=="undefined"&&c.isFunction(j))h=j(c(a).text(),b,a);
...
c.unformat.select=function(a,b,d,e){
    ...
    if(b.value){
        ...
        if(b){
            ...
            if(c.fmatter.isString(h))for(var f=h.split(";"),i=0,j=0;j<f.length;j++){
            g=f[j].split(":");
...
var showModal=function(a){
    ...
    if(jQuery.fn.jqm&&b.jqm===true)jQuery(a).attr("aria-hidden","true").jqmHide();
    else{
        if(b.gb!=="")try{
...
var createModal=function(a,b,c,e,f,g){
    ...
    f=0;
    if(h&&b.left&&!g){
        f=jQuery(c.gbox).width()-(!isNaN(c.width)?parseInt(c.width,10):0)-8;
    ...
    if(c.drag){
        ...
        if(jQuery.fn.jqDrag)jQuery(d).jqDrag(i);else try{
            jQuery(d).draggable({
                handle:jQuery("#"+i.id)
                })
            }catch(q){}
    ...
    if(jQuery.fn.jqm&&b.jqm===true)b.jqM?jQuery(a).attr("aria-hidden",
        "false").jqm(b).jqmShow():jQuery(a).attr("aria-hidden","false").jqmShow();
    else{
        if(b.gbox!==""){
...
function info_dialog(a,b,c,e){
    ...
    c+"</a>"+a+"</div>":a!==""?"<div class='ui-widget-content ui-helper-clearfix' style='text-align:"+f.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'>"+a+"</div>":"";
    d+="</div>";
    try{
...
function createEl(a,b,c,e,f){
    function g(k,m){
        if(jQuery.isFunction(m.dataInit)){
            ...
            jQuery.each(m.dataEvents,function(){
                this.data!==undefined?jQuery(k).bind(this.type,this.data,this.fn):jQuery(k).bind(this.type,this.fn)
                });
            delete m.dataEvents
            }
            return m
    ...
    case "select":
        ...
        if(typeof b.dataUrl!="undefined")jQuery.ajax(jQuery.extend({
        url:b.dataUrl,
        type:"GET",
        dataType:"html",
        success:function(k,m){
            ...
            if(typeof b.buildSelect!=
                "undefined"){
                k=b.buildSelect(k);
                k=jQuery(k).html();
                delete b.buildSelect
                }else k=jQuery(k).html();
            if(k){
                jQuery(d).append(k);
                b=g(d,b);
                if(typeof b.size==="undefined")b.size=j?3:1;
                if(j){
                    l=c.split(",");
                    l=jQuery.map(l,function(p){
                        return jQuery.trim(p)
                        })
                    }else l[0]=jQuery.trim(c);
                jQuery(d).attr(b);
                setTimeout(function(){
                    jQuery("option",d).each(function(p){
                        if(p===0)this.selected="";
                        jQuery(this).attr("role","option");
                        if(jQuery.inArray(jQuery.trim(jQuery(this).text()),l)>-1||jQuery.inArray(jQuery.trim(jQuery(this).val()),
                            l)>-1){
    ...
    if(typeof b.value==="function")b.value=b.value();
    if(typeof b.value==="string"){
        e=b.value.split(";");
        for(h=0;h<e.length;h++){
            f=e[h].split(":");
            if(f.length>2)f[1]=jQuery.map(f,function(k,m){
                if(m>0)return k
                    }).join(":");
            ...
            f[0];
            a.innerHTML=f[1];
            if(!j&&(jQuery.trim(f[0])==jQuery.trim(c)||jQuery.trim(f[1])==jQuery.trim(c)))a.selected="selected";
            if(j&&(jQuery.inArray(jQuery.trim(f[1]),l)>-1||jQuery.inArray(jQuery.trim(f[0]),l)>-1))a.selected="selected";
            d.appendChild(a)
            }
    ...
    for(h in e)if(e.hasOwnProperty(h)){
        ...
        a.value=h;
        a.innerHTML=e[h];
        if(!j&&(jQuery.trim(h)==jQuery.trim(c)||jQuery.trim(e[h])==jQuery.trim(c)))a.selected=
            "selected";
        if(j&&(jQuery.inArray(jQuery.trim(e[h]),l)>-1||jQuery.inArray(jQuery.trim(h),l)>-1))a.selected="selected";
        d.appendChild(a)
        }
    }
    b=g(d,b);
...
case "custom":
    ...
    n=="e2"?info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_element' "+
...
function checkDate(a,b){
    ...
    for(var f,g=-1,d=-1,h=0;h<a.length;h++){
        ...
        c[a[h]]=f;
        f=a[h];
        if(f.indexOf("y")!=-1)e=h;
        if(f.indexOf("m")!=-1)d=h;
        if(f.indexOf("d")!=-1)g=h
            }
    ...
    if(e===-1)return false;
        ...
        i=c[a[g]].toString();
        if(i.length<1||c[a[g]]<1||c[a[g]]>31||c[a[d]]==2&&c[a[g]]>daysInFebruary(c[a[e]])||c[a[g]]>h[c[a[d]]])return false
            }
            return true
    }
    function isEmpty(a){
...
function checkValues(a,b,c){
    ...
    if(typeof b=="string"){
        f=0;
        for(d=c.p.colModel.length;f<d;f++)if(c.p.colModel[f].name==b){
...
if(e){
    g||(g=c.p.colNames[b]);
    if(e.required===true)if(isEmpty(a))return[false,g+": "+jQuery.jgrid.edit.msg.required,""];
    f=e.required===false?false:true;
    if(e.number===true)if(!(f===false&&isEmpty(a)))if(isNaN(a))return[false,g+": "+jQuery.jgrid.edit.msg.number,
        ""];
    if(typeof e.minValue!="undefined"&&!isNaN(e.minValue))if(parseFloat(a)<parseFloat(e.minValue))return[false,g+": "+jQuery.jgrid.edit.msg.minValue+" "+e.minValue,""];
    if(typeof e.maxValue!="undefined"&&!isNaN(e.maxValue))if(parseFloat(a)>parseFloat(e.maxValue))return[false,g+": "+jQuery.jgrid.edit.msg.maxValue+" "+e.maxValue,""];
    if(e.email===true)if(!(f===false&&isEmpty(a))){
        ...
        if(!d.test(a))return[false,g+": "+jQuery.jgrid.edit.msg.email,""]
            }
            if(e.integer===true)if(!(f===false&&isEmpty(a))){
        if(isNaN(a))return[false,g+": "+jQuery.jgrid.edit.msg.integer,""];
        if(a%1!==0||a.indexOf(".")!=-1)return[false,g+": "+jQuery.jgrid.edit.msg.integer,""]
            }
            if(e.date===true)if(!(f===false&&isEmpty(a))){
        b=c.p.colModel[b].formatoptions&&c.p.colModel[b].formatoptions.newformat?c.p.colModel[b].formatoptions.newformat:c.p.colModel[b].datefmt||"Y-m-d";
        if(!checkDate(b,a))return[false,g+": "+jQuery.jgrid.edit.msg.date+
            " - "+b,""]
            }
            if(e.time===true)if(!(f===false&&isEmpty(a)))if(!checkTime(a))return[false,g+": "+jQuery.jgrid.edit.msg.date+" - hh:mm (am/pm)",""];
    if(e.url===true)if(!(f===false&&isEmpty(a))){
        d=/^(((https?)|(ftp)):\/\/([\-\w]+\.)+\w{2,3}(\/[%\-\w]+(\.\w{2,})?)*(([\w\-\.\?\\\/+@&#;`~=%!]*)(\.\w{2,})?)*\/?)/i;
        if(!d.test(a))return[false,g+": "+jQuery.jgrid.edit.msg.url,""]
            }
...
(function(a){
    ...
    a.jgrid.extend({
        searchGrid:function(f){
            ...
            return this.each(function(){
                function b(m,
                    ...
                    if(typeof q=="string")q=a.jgrid.parse(q);
                    if(q){
                        ...
                        if(q.rules){
                            ...
                            for(A=false;F<K;F++){
                                A=q.rules[F];
                                if(A.field!==undefined&&A.op!==undefined&&A.data!==undefined)(A=m.SearchFilter.setFilter({
            ...
            if(f.onClose){
                var q=f.onClose(m);
                if(typeof q=="boolean"&&!q)return
            }
            m.hide();
            f.overlay===true&&a(".jqgrid-overlay:first",
                "#gbox_"+w.p.id).hide()
            }
            function C(){
            ...
            if(m>1){
                ...
                a("#"+i).show();
            f.overlay===true&&a(".jqgrid-overlay:first","#gbox_"+w.p.id).show();
            try{
        ...
        q[0].p.search=false;
        if(f.multipleSearch===false)A[f.sField]=A[f.sValue]=A[f.sOper]="";else A[f.sFilter]="";
    ...
    if(w.grid){
        ...
        if(a.fn.searchFilter){
            ...
            if(a("#"+i).html()!=null){
                ...
                C();
                a.isFunction(f.afterShowSearch)&&f.afterShowSearch(a("#"+i))
                }else{
                var n=[],E=a("#"+w.p.id).jqGrid("getGridParam","colNames"),g=a("#"+w.p.id).jqGrid("getGridParam","colModel"),k=["eq","ne","lt","le","gt","ge","bw","bn","in","ni","ew","en","cn","nc"],h,j,l,s=[];
                if(f.sopt!==null)for(h=l=0;h<f.sopt.length;h++){
            ...
            a.each(g,function(m,q){
                var A=
                typeof q.search==="undefined"?true:q.search,F=q.hidden===true;
                m=a.extend({},{
                    text:E[m],
                ...
                if(typeof m.sopt!=="undefined"){
                    l=0;
                    m.ops=[];
                    if(m.sopt.length>0)for(h=0;h<m.sopt.length;h++)if((j=a.inArray(m.sopt[h],k))!=-1){
                        m.ops[l]={
                            op:m.sopt[h],
                            text:f.odata[j]
                            };
                ...
                if(this.stype=="select")if(m.dataUrl===undefined){
                var K;
...
if(n.length>0){
    a("<div id='"+i+"' role='dialog' tabindex='-1'></div>").insertBefore("#gview_"+w.p.id);
    if(f.stringResult===undefined)f.stringResult=f.multipleSearch;
    w.SearchFilter=a("#"+i).searchFilter(n,{
        groupOps:f.groupOps,
        operators:s,
        onClose:o,
        resetText:f.Reset,
        ...
        ajaxSelectOptions:a.extend({},a.jgrid.ajaxOptions,w.p.ajaxSelectOptions||{}),
        clone:f.cloneSearchRowOnAdd
        });
    a(".ui-widget-overlay","#"+i).remove();
    w.p.direction=="rtl"&&a(".ui-closer","#"+i).css("float","left");
    if(f.drag===true){
        a("#"+i+" table thead tr:first td:first").css("cursor","move");
        if(jQuery.fn.jqDrag)a("#"+i).jqDrag(a("#"+
            i+" table thead tr:first td:first"));else try{
            a("#"+i).draggable({
                handle:a("#"+i+" table thead tr:first td:first")
                })
            }catch(Q){}
        }
        if(f.multipleSearch===false){
    a(".ui-del, .ui-add, .ui-del, .ui-add-last, .matchText, .rulesText","#"+i).hide();
    a("select[name='groupOp']","#"+i).hide()
    }
    f.multipleSearch===true&&f.loadDefaults===true&&b(w,f);
a.isFunction(f.onInitializeSearch)&&f.onInitializeSearch(a("#"+i));
a.isFunction(f.beforeShowSearch)&&f.beforeShowSearch(a("#"+i));
C();
a.isFunction(f.afterShowSearch)&&
f.afterShowSearch(a("#"+i));
f.closeOnEscape===true&&a("#"+i).keydown(function(m){
    m.which==27&&o(a("#"+i))
    })
}
}
...
editGridRow:function(f,b){
            ...
            a(".FormElement","#"+j).each(function(){
                ...
                if(e.length){
                    ...
                    a.each(g.p.colModel,function(){
                        if(this.name==r&&this.editoptions&&a.isFunction(this.editoptions.custom_value)){
                            try{
                                c[r]=this.editoptions.custom_value(a("#"+r,"#"+j),"get");
                                if(c[r]===undefined)throw"e1";
                            }catch(p){
                ...
                switch(a(this).get(0).type){
                    case "checkbox":
                        if(a(this).attr("checked"))c[this.name]=
                        a(this).val();
                    else{
                    ...
                    case "select-one":
                        ...
                        u[this.name]=a("option:selected",this).text();
                        break;
    ...
    function C(e,r,t,p){
        for(var I,z,x,J=0,y,L,G,R=[],H=false,ca="",S=1;S<=p;S++)ca+="<td class='CaptionTD ui-widget-content'>&#160;</td><td class='DataTD ui-widget-content' style='white-space:pre'>&#160;</td>";
        if(e!="_empty")H=a(r).jqGrid("getInd",e);
        a(r.p.colModel).each(function(T){
            I=this.name;
            L=(z=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false)?"style='display:none'":"";
            if(I!=="cb"&&I!=="subgrid"&&this.editable===
                true&&I!=="rn"){
                if(H===false)y="";
                else if(I==r.p.ExpandColumn&&r.p.treeGrid===true)y=a("td:eq("+T+")",r.rows[H]).text();else try{
                    y=a.unformat(a("td:eq("+T+")",r.rows[H]),{
                        rowId:e,
                        colModel:this
                    },T)
                    }catch(da){
                    ...
                    rowabove:false,
                    rowcontent:""
                },this.formoptions||{}),aa=parseInt(X.rowpos,10)||J+1,ea=parseInt((parseInt(X.colpos,10)||1)*2,10);
                if(e=="_empty"&&W.defaultValue)y=a.isFunction(W.defaultValue)?
                    W.defaultValue():W.defaultValue;
                if(!this.edittype)this.edittype="text";
                if(g.p.autoencode)y=a.jgrid.htmlDecode(y);
                G=createEl(this.edittype,W,y,false,a.extend({},a.jgrid.ajaxOptions,r.p.ajaxSelectOptions||{}));
                if(y===""&&this.edittype=="checkbox")y=a(G).attr("offval");
                if(y===""&&this.edittype=="select")y=a("option:eq(0)",G).text();
                if(d.checkOnSubmit||d.checkOnUpdate)d._savedData[I]=y;
                a(G).addClass("FormElement");
                x=a(t).find("tr[rowpos="+aa+"]");
                if(X.rowabove){
                    W=a("<tr><td class='contentinfo' colspan='"+
                        p*2+"'>"+X.rowcontent+"</td></tr>");
                    a(t).append(W);
                    W[0].rp=aa
                    }
                    if(x.length===0){
                    x=a("<tr "+L+" rowpos='"+aa+"'></tr>").addClass("FormData").attr("id","tr_"+I);
                    a(x).append(ca);
                    a(t).append(x);
                    x[0].rp=aa
                    }
                    a("td:eq("+(ea-2)+")",x[0]).html(typeof X.label==="undefined"?r.p.colNames[T]:X.label);
                a("td:eq("+(ea-1)+")",x[0]).append(X.elmprefix).append(G).append(X.elmsuffix);
                R[J]=T;
                J++
            }
        });
    if(J>0){
        S=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(p*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='"+
            r.p.id+"_id' value='"+e+"'/></td></tr>");
        S[0].rp=J+999;
        a(t).append(S);
        if(d.checkOnSubmit||d.checkOnUpdate)d._savedData[r.p.id+"_id"]=e
            }
            return R
    }
    function v(e,r,t){
    ...
    if(e=="_empty"){
        a(G).each(function(){
            ...
            if(x[0]!=null){
                y="";
                if(J.defaultValue){
                    y=a.isFunction(J.defaultValue)?J.defaultValue():J.defaultValue;
                    ...
                    y=a(x).attr("offval")
                    }else if(x[0].type&&x[0].type.substr(0,6)=="select")x[0].selectedIndex=0;else x.val(y);
                if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[p]=y
                    }
                });
    a("#id_g","#"+t).val(e)
    }else{
    var R=a(r).jqGrid("getInd",
        e,true);
    if(R){
        a("td",R).each(function(H){
            p=G[H].name;
            if(p!=="cb"&&p!=="subgrid"&&p!=="rn"&&G[H].editable===true){
                if(p==r.p.ExpandColumn&&r.p.treeGrid===true)z=a(this).text();else try{
                    z=a.unformat(a(this),{
                        rowId:e,
                        colModel:G[H]
                        },H)
                    ...
                    if(g.p.autoencode)z=a.jgrid.htmlDecode(z);
                if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[p]=z;
                p=a.jgrid.jqID(p);
                switch(G[H].edittype){
                    case "password":case "text":case "button":case "image":
                        a("#"+p,"#"+t).val(z);
                        break;
                    case "textarea":
                        if(z==
                        "&nbsp;"||z=="&#160;"||z.length==1&&z.charCodeAt(0)==160)z="";
                        a("#"+p,"#"+t).val(z);
                        break;
                    case "select":
                        var S=z.split(",");
                    ...
                    a("#"+p+" option","#"+t).each(function(){
                        this.selected=!G[H].editoptions.multiple&&(S[0]==a.trim(a(this).text())||S[0]==a.trim(a(this).val()))?true:G[H].editoptions.multiple?a.inArray(a.trim(a(this).text()),S)>-1||a.inArray(a.trim(a(this).val()),S)>-1?true:false:false
                        });
                    break;
                    case "checkbox":
                        z+="";
                        if(G[H].editoptions&&G[H].editoptions.value)if(G[H].editoptions.value.split(":")[0]==
                        z){
                        a("#"+p,"#"+t).attr("checked",true);
                        a("#"+p,"#"+t).attr("defaultChecked",true)
                        }else{
                        a("#"+p,"#"+t).attr("checked",false);
                        a("#"+p,"#"+t).attr("defaultChecked","")
                        }else{
                        z=z.toLowerCase();
                ...
                case "custom":
                    try{
                    if(G[H].editoptions&&a.isFunction(G[H].editoptions.custom_value))G[H].editoptions.custom_value(a("#"+
                        p,"#"+t),"set",z);else throw"e1";
                }catch(T){
                    T=="e1"?info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose):info_dialog(jQuery.jgrid.errors.errcap,T.message,jQuery.jgrid.edit.bClose)
                    }
                    break
                    }
                    I++
...
function B(){
    ...
    if(a.isFunction(d.beforeCheckValues)){
        var z=d.beforeCheckValues(c,a("#"+h),c[g.p.id+"_id"]=="_empty"?t.addoper:t.editoper);
        if(z&&typeof z==="object")c=z
            }
    ...
    if(e[0]){
        if(a.isFunction(d.onclickSubmit))r=d.onclickSubmit(d,c)||{};
            
        if(a.isFunction(d.beforeSubmit))e=d.beforeSubmit(c,a("#"+h))
            }
            if(e[0]&&!d.processing){
        d.processing=true;
        ...
        I=t.oper;
        p=t.id;
        c[I]=a.trim(c[g.p.id+"_id"])=="_empty"?t.addoper:t.editoper;
        if(c[I]!=t.addoper)c[p]=c[g.p.id+"_id"];
        else if(c[p]===undefined)c[p]=c[g.p.id+"_id"];
        delete c[g.p.id+"_id"];
        c=a.extend(c,d.editData,r);
        r=a.extend({
            url:d.url?
            d.url:a(g).jqGrid("getGridParam","editurl"),
            type:d.mtype,
            data:a.isFunction(d.serializeEditData)?d.serializeEditData(c):c,
            complete:function(J,y){
                if(y!="success"){
                    e[0]=false;
                    e[1]=a.isFunction(d.errorTextFormat)?d.errorTextFormat(J):y+" Status: '"+J.statusText+"'. Error code: "+J.status
                    }else if(a.isFunction(d.afterSubmit))e=d.afterSubmit(J,c);
                if(e[0]===false){
                    a("#FormError>td","#"+j).html(e[1]);
                    a("#FormError","#"+j).show()
                    }else{
                    a.each(g.p.colModel,function(){
                        if(u[this.name]&&this.formatter&&this.formatter==
                            "select")try{
                            delete u[this.name]
                        }catch(R){}
                        });
                ...
                d.reloadAfterSubmit=d.reloadAfterSubmit&&g.p.datatype!="local";
                if(c[I]==t.addoper){
                    e[2]||(e[2]=parseInt(g.p.records,10)+1+"");
                    c[p]=e[2];
                    if(d.closeAfterAdd){
                        if(d.reloadAfterSubmit)a(g).trigger("reloadGrid");
                        else{
                            a(g).jqGrid("addRowData",e[2],c,b.addedrow);
                        ...
                        d.reloadAfterSubmit?
                        a(g).trigger("reloadGrid"):a(g).jqGrid("addRowData",e[2],c,b.addedrow);
                        v("_empty",g,h)
                        }else d.reloadAfterSubmit?a(g).trigger("reloadGrid"):a(g).jqGrid("addRowData",e[2],c,b.addedrow)
                        }else{
                    if(d.reloadAfterSubmit){
                        ...
                        }else g.p.treeGrid===true?a(g).jqGrid("setTreeRow",c[p],c):a(g).jqGrid("setRowData",c[p],c);
                    d.closeAfterEdit&&hideModal("#"+l.themodal,{
                        gb:"#gbox_"+k,
                        jqm:b.jqModal,
                        onClose:d.onClose
                        })
        ...
        if(d.checkOnSubmit||d.checkOnUpdate){
            ...
            if(d._savedData[g.p.id+"_id"]!="_empty")for(var L in d._savedData)if(c[L])d._savedData[L]=c[L]
                }
...
if(!r.url&&!d.useDataProxy)if(a.isFunction(g.p.dataProxy))d.useDataProxy=true;
    else{
    e[0]=false;
    e[1]+=" "+a.jgrid.errors.nourl
    }
    if(e[0])d.useDataProxy?g.p.dataProxy.call(g,r,"set_"+g.p.id):a.ajax(r)
    }
    if(e[0]===false){
    a("#FormError>td","#"+j).html(e[1]);
    a("#FormError","#"+j).show()
...
function n(e,r){
    e===0?a("#pData","#"+j+"_2").addClass("ui-state-disabled"):a("#pData","#"+j+"_2").removeClass("ui-state-disabled");
    e==r?a("#nData","#"+j+"_2").addClass("ui-state-disabled"):a("#nData","#"+j+"_2").removeClass("ui-state-disabled")
    }
    function E(){
    var e=a(g).jqGrid("getDataIDs"),r=a("#id_g","#"+j).val();
...
if(g.grid&&f){
    var k=g.p.id,h="FrmGrid_"+k,j="TblGrid_"+k,l={
        ...
        scrollelm:h
    },s=a.isFunction(d.beforeShowForm)?d.beforeShowForm:false,Q=a.isFunction(d.afterShowForm)?d.afterShowForm:false,m=a.isFunction(d.beforeInitData)?d.beforeInitData:false,q=a.isFunction(d.onInitializeForm)?d.onInitializeForm:false,A=null,F=1,K=0,c,u,M,U;
    if(f=="new"){
        f="_empty";
        b.caption=b.addCaption
        }else b.caption=b.editCaption;
    b.recreateForm===true&&a("#"+
        l.themodal).html()!=null&&a("#"+l.themodal).remove();
    var O=true;
    if(b.checkOnUpdate&&b.jqModal&&!b.modal)O=false;
    if(a("#"+l.themodal).html()!=null){
        ...
        v(f,g,
            h);
        f=="_empty"||!d.viewPagerButtons?a("#pData, #nData","#"+j+"_2").hide():a("#pData, #nData","#"+j+"_2").show();
        ...
        var P=isNaN(b.dataheight)?b.dataheight:b.dataheight+"px";
        P=a("<form name='FormPost' id='"+h+"' class='FormGrid' onSubmit='return false;' style='width:100%;overflow:auto;position:relative;height:"+P+";'></form>").data("disabled",false);
        var N=a("<table id='"+j+"' class='EditTable' cellspacing='0' cellpading='0' border='0'><tbody></tbody></table>");
        a(P).append(N);
        var D=a("<tr id='FormError' style='display:none'><td class='ui-state-error' colspan='"+F*2+"'></td></tr>");
        D[0].rp=0;
        a(N).append(D);
        D=a("<tr style='display:none' class='tinfo'><td class='topinfo' colspan='"+F*2+"'>"+d.topinfo+"</td></tr>");
        D[0].rp=0;
        a(N).append(D);
        m&&m(a("#"+h));
        D=(m=g.p.direction=="rtl"?true:false)?"nData":"pData";
        var V=m?"pData":"nData";
        C(f,g,N,F);
        ...
        D="<table border='0' class='EditTable' id='"+j+"_2'><tbody><tr id='Act_Buttons'><td class='navButton ui-widget-content'>"+(m?
            V+D:D+V)+"</td><td class='EditButton ui-widget-content'>"+Z+$+"</td></tr>";
        D+="<tr style='display:none' class='binfo'><td class='bottominfo' colspan='2'>"+d.bottominfo+"</td></tr>";
        D+="</tbody></table>";
        if(K>0){
            var Y=[];
        ...
        createModal(l,P,b,"#gview_"+g.p.id,a("#gbox_"+g.p.id)[0]);
        if(m){
        ...
        a("#"+l.themodal).keydown(function(e){
            ...
            if(d.savekey[0]===true&&e.which==d.savekey[1])if(r.tagName!="TEXTAREA"){
...
if(b.checkOnUpdate){
    a("a.ui-jqdialog-titlebar-close span","#"+l.themodal).removeClass("jqmClose");
    a("a.ui-jqdialog-titlebar-close","#"+l.themodal).unbind("click").click(function(){
        if(!i())return false;
        hideModal("#"+l.themodal,{
            gb:"#gbox_"+k,
            jqm:b.jqModal,
...
a("#nData","#"+j+"_2").click(function(){
    ...
    e[0]=parseInt(e[0],10);
    if(e[0]!=-1&&e[1][e[0]+1]){
        a.isFunction(b.onclickPgButtons)&&b.onclickPgButtons("next",a("#"+h),e[1][e[0]]);
        v(e[1][e[0]+1],g,h);
        a(g).jqGrid("setSelection",e[1][e[0]+1]);
        a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons("next",a("#"+h),e[1][e[0]+1]);
...
a("#pData","#"+j+"_2").click(function(){
    ...
    if(e[0]!=-1&&e[1][e[0]-1]){
        ...
        b.onclickPgButtons("prev",a("#"+h),e[1][e[0]]);
        v(e[1][e[0]-1],g,h);
        a(g).jqGrid("setSelection",e[1][e[0]-1]);
        a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons("prev",a("#"+h),e[1][e[0]-1]);
        n(e[0]-1,e[1].length-1)
        }
        return false
    })
...
viewGridRow:function(f,b){
    ...
    return this.each(function(){
        function o(){
            ...
            function C(c,u,M,U){
            for(var O,P,N,D=0,V,Z,$=[],Y=false,ba="<td class='CaptionTD form-view-label ui-widget-content' width='"+b.labelswidth+"'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>",e="",r=["integer","number","currency"],t=0,p=0,I,z,x,J=
                1;J<=U;J++)e+=J==1?ba:"<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>";
            a(u.p.colModel).each(function(){
                P=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false;
                if(!P&&this.align==="right")if(this.formatter&&a.inArray(this.formatter,r)!==-1)t=Math.max(t,parseInt(this.width,10));else p=Math.max(p,parseInt(this.width,10))
                    });
            I=t!==0?t:p!==0?p:0;
            Y=a(u).jqGrid("getInd",c);
            a(u.p.colModel).each(function(y){
                O=
                this.name;
                z=false;
                Z=(P=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false)?"style='display:none'":"";
                x=typeof this.viewable!="boolean"?true:this.viewable;
                if(O!=="cb"&&O!=="subgrid"&&O!=="rn"&&x){
                    V=Y===false?"":O==u.p.ExpandColumn&&u.p.treeGrid===true?a("td:eq("+y+")",u.rows[Y]).text():a("td:eq("+y+")",u.rows[Y]).html();
                    z=this.align==="right"&&I!==0?true:false;
                    ...
                    {}),G=parseInt(L.rowpos,10)||D+1,R=parseInt((parseInt(L.colpos,10)||1)*2,10);
                    if(L.rowabove){
                        var H=a("<tr><td class='contentinfo' colspan='"+U*2+"'>"+L.rowcontent+"</td></tr>");
                        a(M).append(H);
                        H[0].rp=G
                        }
                        N=a(M).find("tr[rowpos="+G+"]");
                    if(N.length===0){
                        N=a("<tr "+Z+" rowpos='"+G+"'></tr>").addClass("FormData").attr("id","trv_"+O);
                        a(N).append(e);
                        a(M).append(N);
                        N[0].rp=G
                        }
                        a("td:eq("+(R-2)+")",N[0]).html("<b>"+(typeof L.label==="undefined"?u.p.colNames[y]:L.label)+"</b>");
                    a("td:eq("+(R-1)+")",N[0]).append("<span>"+
                        V+"</span>").attr("id","v_"+O);
        ...
        if(D>0){
            c=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(U*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+c+"'/></td></tr>");
            c[0].rp=D+99;
            a(M).append(c)
            }
            return $
        ...
        if(N=a(u).jqGrid("getInd",c,true)){
            a("td",N).each(function(D){
                M=u.p.colModel[D].name;
                U=u.p.colModel[D].editrules&&
                u.p.colModel[D].editrules.edithidden===true?false:u.p.colModel[D].hidden===true?true:false;
                if(M!=="cb"&&M!=="subgrid"&&M!=="rn"){
                    P=M==u.p.ExpandColumn&&u.p.treeGrid===true?a(this).text():a(this).html();
                    a.extend({},u.p.colModel[D].editoptions||{});
                    M=a.jgrid.jqID("v_"+M);
                    a("#"+M+" span","#"+g).html(P);
                    U&&a("#"+M,"#"+g).parents("tr:first").hide();
                    O++
                }
            });
        O>0&&a("#id_g","#"+g).val(c)
    ...
    function B(c,u){
    c===0?a("#pData","#"+g+"_2").addClass("ui-state-disabled"):a("#pData","#"+g+"_2").removeClass("ui-state-disabled");
    c==u?a("#nData","#"+g+"_2").addClass("ui-state-disabled"):a("#nData","#"+g+"_2").removeClass("ui-state-disabled")
    }
    function w(){
    var c=a(i).jqGrid("getDataIDs"),u=a("#id_g","#"+g).val();
    return[a.inArray(u,c),c]
    }
    var i=this;
if(i.grid&&f){
    if(!b.imgpath)b.imgpath=i.p.imgpath;
    ...
    if(a("#"+k.themodal).html()!=null){
        ...
        a("#FormError",
            "#"+g).hide();
        v(f,i);
        a.isFunction(b.beforeShowForm)&&b.beforeShowForm(a("#"+E));
        ...
        var l=isNaN(b.dataheight)?b.dataheight:b.dataheight+"px",s=a("<form name='FormPost' id='"+E+"' class='FormGrid' style='width:100%;overflow:auto;position:relative;height:"+l+";'></form>"),Q=a("<table id='"+g+"' class='EditTable' cellspacing='1' cellpading='2' border='0' style='table-layout:fixed'><tbody></tbody></table>");
        a(s).append(Q);
        C(f,i,Q,h);
        l=i.p.direction=="rtl"?true:false;
        var m="<a href='javascript:void(0)' id='"+(l?"nData":"pData")+"' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>",q="<a href='javascript:void(0)' id='"+(l?"pData":"nData")+"' class='fm-button ui-state-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>",A="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default ui-corner-all'>"+b.bClose+
        "</a>";
        if(j>0){
            var F=[];
        ...
        if(b.closeOnEscape===true){
            ...
            s=a("<span></span>").append(s).append("<table border='0' class='EditTable' id='"+g+"_2'><tbody><tr id='Act_Buttons'><td class='navButton ui-widget-content' width='"+b.labelswidth+"'>"+(l?q+m:m+q)+"</td><td class='EditButton ui-widget-content'>"+
            A+"</td></tr></tbody></table>");
        createModal(k,s,b,"#gview_"+i.p.id,a("#gview_"+i.p.id)[0]);
        if(l){
            a("#pData, #nData","#"+g+"_2").css("float","right");
            a(".EditButton","#"+g+"_2").css("text-align","left")
            }
            b.viewPagerButtons||a("#pData, #nData","#"+g+"_2").hide();
        s=null;
...
a(".fm-button:not(.ui-state-disabled)","#"+g+"_2").hover(function(){
    a(this).addClass("ui-state-hover")
    },
function(){
    a(this).removeClass("ui-state-hover")
...
a("#nData","#"+g+"_2").click(function(){
    ...
    c[0]=parseInt(c[0],10);
    if(c[0]!=-1&&c[1][c[0]+1]){
        a.isFunction(b.onclickPgButtons)&&b.onclickPgButtons("next",a("#"+E),c[1][c[0]]);
        v(c[1][c[0]+1],i);
        a(i).jqGrid("setSelection",c[1][c[0]+1]);
        a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons("next",
            a("#"+E),c[1][c[0]+1]);
...
a("#pData","#"+g+"_2").click(function(){
    ...
    var c=w();
    if(c[0]!=-1&&c[1][c[0]-1]){
        a.isFunction(b.onclickPgButtons)&&b.onclickPgButtons("prev",a("#"+E),c[1][c[0]]);
        v(c[1][c[0]-1],i);
        a(i).jqGrid("setSelection",c[1][c[0]-1]);
        a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons("prev",a("#"+E),c[1][c[0]-1]);
        B(c[0]-1,c[1].length-1)
        }
        o();
    return false
...
delGridRow:function(f,b){
    ...
    return this.each(function(){
        ...
        if(o.grid)if(f){
            ...
            "function"?true:false,v=typeof b.afterShowForm==="function"?true:false,B=o.p.id,w={},i="DelTbl_"+B,n,E,g,k,h={
                themodal:"delmod"+B,
                modalhead:"delhd"+B,
            ...
            if(a("#"+h.themodal).html()!=null){
                ...
                j="<div id='"+i+"' class='formdata' style='width:100%;overflow:auto;position:relative;height:"+j+";'>";
                j+="<table class='DelTable'><tbody>";
                j+="<tr id='DelError' style='display:none'><td class='ui-state-error'></td></tr>";
                j+="<tr id='DelData' style='display:none'><td >"+f+"</td></tr>";
                j+='<tr><td class="delmsg" style="white-space:pre;">'+b.msg+"</td></tr><tr><td >&#160;</td></tr>";
                j+="</tbody></table></div>";
                ...
                b.gbox="#gbox_"+B;
                createModal(h,j,b,"#gview_"+o.p.id,a("#gview_"+o.p.id)[0]);
                a(".fm-button",
                    "#"+i+"_2").hover(function(){
                    a(this).addClass("ui-state-hover")
                    },function(){
                ...
                if(b.delicon[0]===true)a("#dData","#"+i+"_2").addClass(b.delicon[1]=="right"?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+b.delicon[2]+"'></span>");
                if(b.cancelicon[0]===true)a("#eData","#"+i+"_2").addClass(b.cancelicon[1]==
                    "right"?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+b.cancelicon[2]+"'></span>");
                a("#dData","#"+i+"_2").click(function(){
                    var l=[true,""];
                    w={};
                    
                    var s=a("#DelData>td","#"+i).text();
                    if(typeof b.onclickSubmit==="function")w=b.onclickSubmit(d,s)||{};
                        
                    if(typeof b.beforeSubmit==="function")l=b.beforeSubmit(s);
                    if(l[0]&&!d.processing){
                        d.processing=true;
                        ...
                        var Q=a.extend({
                            url:d.url?
                            d.url:a(o).jqGrid("getGridParam","editurl"),
                            type:b.mtype,
                            data:a.isFunction(b.serializeDelData)?b.serializeDelData(n):n,
                            complete:function(m,q){
                                if(q!="success"){
                                    l[0]=false;
                                    l[1]=a.isFunction(d.errorTextFormat)?d.errorTextFormat(m):q+" Status: '"+m.statusText+"'. Error code: "+m.status
                                    }else if(typeof d.afterSubmit==="function")l=d.afterSubmit(m,n);
                                if(l[0]===false){
                                    a("#DelError>td","#"+i).html(l[1]);
                                    a("#DelError","#"+i).show()
                                    }else{
                                    if(d.reloadAfterSubmit&&o.p.datatype!="local")a(o).trigger("reloadGrid");
                                    else{
                                        q=[];
                                        q=s.split(",");
                                        if(o.p.treeGrid===true)try{
                                            a(o).jqGrid("delTreeNode",q[0])
                                            }catch(A){}else for(var F=0;F<q.length;F++)a(o).jqGrid("delRowData",q[F]);
                                        o.p.selrow=null;
                                        o.p.selarrrow=[]
                                        }
                    ...
                    if(!Q.url&&!d.useDataProxy)if(a.isFunction(o.p.dataProxy))d.useDataProxy=true;
                        else{
                        l[0]=false;
                        l[1]+=" "+a.jgrid.errors.nourl
                        }
                        if(l[0])d.useDataProxy?o.p.dataProxy.call(o,Q,"del_"+o.p.id):a.ajax(Q)
                        }
                        if(l[0]===false){
                    a("#DelError>td","#"+i).html(l[1]);
                    a("#DelError","#"+i).show()
            ...
            a(".ui-jqdialog-titlebar-close","#"+h.modalhead).focus()
            },0)
        }
        })
},
navGrid:function(f,b,o,C,v,B,w){
    ...
    return this.each(function(){
        ...
        if(!(!n.grid||typeof f!="string")){
            if(a("#"+i.themodal).html()===null){
                if(typeof window.innerWidth!="undefined"){
                    ...
                    createModal(i,"<div>"+b.alerttext+"</div><span tabindex='0'><span tabindex='-1' id='jqg_alrt'></span></span>",{
                    gbox:"#gbox_"+n.p.id,
                    jqModal:true,
                    drag:true,
                    resize:true,
                ...
                E=1;
            if(b.cloneToTop&&n.p.toppager)E=2;
            for(g=0;g<E;g++){
                var h=a("<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table navtable' style='float:left;table-layout:auto;'><tbody><tr></tr></tbody></table>"),
                j,l;
                if(g===0){
                    j=f;
                    l=n.p.id;
                ...
                n.p.direction=="rtl"&&a(h).attr("dir","rtl").css("float","right");
                if(b.add){
                C=C||{};
                
                k=a("<td class='ui-pg-button ui-corner-all'></td>");
                a(k).append("<div class='ui-pg-div'><span class='ui-icon "+b.addicon+"'></span>"+b.addtext+"</div>");
                a("tr",h).append(k);
                a(k,h).attr({
                    title:b.addtitle||"",
                    id:C.id||"add_"+l
                ...
                a(k).append("<div class='ui-pg-div'><span class='ui-icon "+b.editicon+"'></span>"+b.edittext+"</div>");
                a("tr",h).append(k);
                a(k,h).attr({
                    title:b.edittitle||"",
                    id:o.id||"edit_"+l
                    ...
                    if(!a(this).hasClass("ui-state-disabled")){
                        var s=
                        n.p.selrow;
                        if(s)typeof b.editfunc=="function"?b.editfunc(s):a(n).jqGrid("editGridRow",s,o);
                        else{
            ...
            if(b.view){
                ...
                a(k).append("<div class='ui-pg-div'><span class='ui-icon "+b.viewicon+"'></span>"+b.viewtext+
                    "</div>");
                a("tr",h).append(k);
                a(k,h).attr({
                    title:b.viewtitle||"",
                    ...
                    if(!a(this).hasClass("ui-state-disabled")){
                        var s=n.p.selrow;
                        if(s)a(n).jqGrid("viewGridRow",s,w);
                        else{
                            viewModal("#"+i.themodal,{
                                gbox:"#gbox_"+n.p.id,
                                jqm:true
            ...
            a(k).append("<div class='ui-pg-div'><span class='ui-icon "+b.delicon+"'></span>"+b.deltext+"</div>");
            a("tr",h).append(k);
            a(k,h).attr({
                title:b.deltitle||"",
                id:v.id||"del_"+l
                ...
                if(!a(this).hasClass("ui-state-disabled")){
                    ...
                    if(s)"function"==typeof b.delfunc?b.delfunc(s):a(n).jqGrid("delGridRow",s,v);
                    else{
                        viewModal("#"+i.themodal,{
                            gbox:"#gbox_"+n.p.id,
                            jqm:true
        ...
        if(b.add||b.edit||b.del||b.view)a("tr",h).append("<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>");
        if(b.search){
        k=a("<td class='ui-pg-button ui-corner-all'></td>");
        B=B||{};
        
        a(k).append("<div class='ui-pg-div'><span class='ui-icon "+b.searchicon+"'></span>"+b.searchtext+"</div>");
        a("tr",h).append(k);
        a(k,h).attr({
            title:b.searchtitle||"",
            id:B.id||"search_"+
            ...
            a(this).hasClass("ui-state-disabled")||a(n).jqGrid("searchGrid",B);
            return false
            }).hover(function(){
            a(this).hasClass("ui-state-disabled")||a(this).addClass("ui-state-hover")
            },function(){
        ...
        if(b.refresh){
        k=a("<td class='ui-pg-button ui-corner-all'></td>");
        a(k).append("<div class='ui-pg-div'><span class='ui-icon "+b.refreshicon+"'></span>"+b.refreshtext+"</div>");
        a("tr",h).append(k);
        a(k,h).attr({
...
navButtonAdd:function(f,b){
    ...
    return this.each(function(){
        if(this.grid){
            ...
            if(o){
                var v=a("<td></td>");
                b.buttonicon.toString().toUpperCase()=="NONE"?a(v).addClass("ui-pg-button ui-corner-all").append("<div class='ui-pg-div'>"+b.caption+"</div>"):a(v).addClass("ui-pg-button ui-corner-all").append("<div class='ui-pg-div'><span class='ui-icon "+b.buttonicon+"'></span>"+b.caption+"</div>");
                b.id&&a(v).attr("id",b.id);
                if(b.position==
                    "first")o.rows[0].cells.length===0?a("tr",o).append(v):a("tr td:eq(0)",o).before(v);else a("tr",o).append(v);
                a(v,o).attr("title",b.title||"").click(function(B){
                    a(this).hasClass("ui-state-disabled")||a.isFunction(b.onClickButton)&&b.onClickButton.call(C,B);
                    return false
                    }).hover(function(){
                    a(this).hasClass("ui-state-disabled")||a(this).addClass("ui-state-hover")
                    },function(){
...
GridToForm:function(f,b){
    return this.each(function(){
        ...
        if(o.grid){
            ...
            if(C)for(var v in C)a("[name="+v+"]",b).is("input:radio")||a("[name="+v+"]",b).is("input:checkbox")?a("[name="+v+"]",b).each(function(){
                a(this).val()==
                C[v]?a(this).attr("checked","checked"):a(this).attr("checked","")
                }):a("[name="+v+"]",b).val(C[v])
                }
            })
},
FormToGrid:function(f,b,o,C){
    return this.each(function(){
        ...
        if(v.grid){
            ...
            if(o=="add")a(v).jqGrid("addRowData",f,w,C);else o=="set"&&a(v).jqGrid("setRowData",f,w)
                }
            })
}
})
                ...
                switch(k.edittype){
                    ...
                    if(r[0]===true){
                        ...
                        if(a.p.cellsubmit=="remote")if(a.p.cellurl){
                            ...
                            b.ajax(b.extend({
                                url:a.p.cellurl,
                                data:b.isFunction(a.p.serializeCellData)?a.p.serializeCellData.call(a,
                                    o):o,
                                type:"POST",
                            ...
                            error:function(m,p){
                                b("#lui_"+a.p.id).hide();
                                a.grid.hDiv.loading=false;
                                b.isFunction(a.p.errorCell)?a.p.errorCell.call(a,m,p):info_dialog(b.jgrid.errors.errcap,m.status+" : "+m.statusText+"<br/>"+p,b.jgrid.edit.bClose);
...
nextCell:function(d,e){
    return this.each(function(){
        var a=this,c=false;
        if(!(!a.grid||a.p.cellEdit!==true)){
            for(var h=e+1;h<a.p.colModel.length;h++)if(a.p.colModel[h].editable===
                true){
                c=h;
                break
            }
    ...
    return this.each(function(){
        function d(g,k,j){
            if(j.substr(0,1)=="v"){
                ...
                l=b(a.grid.bDiv)[0].scrollTop,n=a.rows[g].offsetTop+a.rows[g].clientHeight,q=a.rows[g].offsetTop;
                ...
                if(a.rows[g].cells[k].offsetLeft+a.rows[g].cells[k].clientWidth>=j+parseInt(i,10))b(a.grid.bDiv)[0].scrollLeft=
            ...
            if(!this[c]){
                ...
                b(this).click(function(){
                    for(var h in{
                        jqmShow:1,
                        jqmHide:1
                    })for(var g in this[h])i[this[h][g]]&&i[this[h][g]].w[h](this);return e
                    })
                }
                this[c].push(d)
            })
                ...
                if(a.p.subGridModel[0].params)for(h=0;h<a.p.subGridModel[0].params.length;h++)for(d=0;d<a.p.colModel.length;d++)if(a.p.colModel[d].name==a.p.subGridModel[0].params[h])f[a.p.colModel[d].name]=
                    ...
                    switch(a.p.subgridtype){
                        case "xml":case "json":
                            ...
                            data:b.isFunction(a.p.serializeSubGridData)?a.p.serializeSubGridData(a,f):f,
                            complete:function(i){
                                a.p.subgridtype==
...
(function(f){
    f.jgrid.extend({
        groupingSetup:function(){
            return this.each(function(){
                ...
                if(c!==null&&(typeof c==="object"||f.isFunction(c)))if(c.groupField.length){
                    for(var a=0;a<c.groupField.length;a++){
                        c.groupOrder[a]||(c.groupOrder[a]="asc");
                        c.groupText[a]||(c.groupText[a]="{0}");
                        if(typeof c.groupColumnShow[a]!="boolean")c.groupColumnShow[a]=true;
                        if(typeof c.groupSummary[a]!="boolean")c.groupSummary[a]=false;
                        c.groupColumnShow[a]===true?f(b).jqGrid("showCol",c.groupField[a]):
                        f(b).jqGrid("hideCol",c.groupField[a]);
                        c.sortitems[a]=[];
                        c.sortnames[a]=[];
                        c.summaryval[a]=[];
                        if(c.groupSummary[a]){
                            c.summary[a]=[];
                            for(var d=b.p.colModel,e=0,g=d.length;e<g;e++)d[e].summaryType&&c.summary[a].push({
                                nm:d[e].name,
                                st:d[e].summaryType,
                                v:""
                            })
                ...
                b.p.treeGrid=false;
                b.p.gridview=true
                }else b.p.grouping=false;else b.p.grouping=false
                })
        },
    groupingPrepare:function(b,c,a,d){
        this.each(function(){
            var e=c[0]?c[0].toString().split(" ").join(""):
            "",g=this.p.groupingView,j=this;
            if(a.hasOwnProperty(e))a[e].push(b);
            else{
                a[e]=[];
                a[e].push(b);
    ...
    groupingToggle:function(b){
        this.each(function(){
            var c=this.p.groupingView,a=b.lastIndexOf("_"),
            d=b.substring(0,a+1);
            a=parseInt(b.substring(a+1),10)+1;
            var e=c.minusicon,g=c.plusicon;
            if(f("#"+b+" span").hasClass(e)){
                c.showSummaryOnHide&&c.groupSummary[0]?f("#"+b).nextUntil(".jqfoot").hide():f("#"+b).nextUntil("#"+d+String(a)).hide();
    ...
    groupingRender:function(b,c){
        ...
        f.each(d.sortitems[0],function(h,k){
            j=a.p.id+"ghead_"+h;
            g="<span style='cursor:pointer;' class='ui-icon "+l+"' onclick=\"jQuery('#"+a.p.id+"').jqGrid('groupingToggle','"+j+"');return false;\"></span>";
            e+='<tr id="'+j+'" role="row" class= "ui-widget-content jqgroup ui-row-'+a.p.direction+'"><td colspan="'+c+'">'+g+
            f.jgrid.format(d.groupText[0],d.sortnames[0][h],b[k].length)+"</td></tr>";
            for(h=0;h<b[k].length;h++)e+=b[k][h].join("");
            if(d.groupSummary[0]){
                h="";
                ...
                for(var m=a.p.colModel,n,o=b[k].length,i=0;i<c;i++){
                    var p="<td "+a.formatCol(i,1,"")+">&#160;</td>",q="{0}";
                    f.each(h,function(){
                        if(this.nm==m[i].name){
                            if(m[i].summaryTpl)q=m[i].summaryTpl;
                            ...
                            try{
                                ...
                                p="<td "+a.formatCol(i,1,"")+">"+f.jgrid.format(q,n)+"</td>";
                            return false
                            }
                        });
                e+=p
...
groupingGroupBy:function(b,c){
    return this.each(function(){
        ...
        a.p.groupingView=f.extend(a.p.groupingView,
            c||{});
        d.groupField=b;
...
groupingRemove:function(b){
    return this.each(function(){
        ...
        c.p.grouping=false;
        b===true?f("tr.jqgroup, tr.jqfoot","#"+c.p.id+" tbody:first").remove():f(c).trigger("reloadGrid")
        })
    },
groupingCalculations:{
    ...
    min:function(b,c,a){
        if(b==="")return parseFloat(a[c]||0);
        return Math.min(parseFloat(b),parseFloat(a[c]||0))
        },
    max:function(b,c,a){
        if(b==="")return parseFloat(a[c]||
            0);
            ...
            return this.each(function(){
                ...
                if(a.grid&&a.p.treeGrid){
                    ...
                    if(a.p.treeGridModel=="nested"){
                        ...
                        b[k]||(b[k]=parseInt(h,10)===parseInt(i,10)+1?"true":"false")
                        }
                        h=parseInt(b[j],10);
                    if(a.p.tree_root_level===0){
                        ...
                        i="<div class='tree-wrap tree-wrap-"+
                    a.p.direction+"' style='width:"+i*18+"px;'>";
                    i+="<div style='"+(a.p.direction=="rtl"?"right:":"left:")+h*18+"px;' class='ui-icon ";
                    ...
                    if(!a.p.loadonce){
                        ...
                        if(parseInt(b[j],
                        10)!==parseInt(a.p.tree_root_level,10))d(a).jqGrid("isVisibleNode",b)||d(c).css("display","none");
                    d("td:eq("+f+")",c).wrapInner("<span></span>").prepend(i);
                    d(".treeclick",c).bind("click",function(g){
                        g=d(g.target||g.srcElement,a.rows).closest("tr.jqgrow")[0].id;
                        g=a.p._index[g];
                        var l=a.p.treeReader.expanded_field;
                        if(!a.p.data[g][a.p.treeReader.leaf_field])if(a.p.data[g][l]){
                    ...
                    a.p.ExpandColClick===true&&d("span",c).css("cursor","pointer").bind("click",function(g){
                        g=d(g.target||g.srcElement,a.rows).closest("tr.jqgrow")[0].id;
                        var l=a.p._index[g],m=a.p.treeReader.expanded_field;
                        if(!a.p.data[l][a.p.treeReader.leaf_field])if(a.p.data[l][m]){
...
getNodeParent:function(b){
    ...
    this.each(function(){
        ...
        if(a.grid&&a.p.treeGrid)switch(a.p.treeGridModel){
            case "nested":
                var f=
                a.p.treeReader.left_field,e=a.p.treeReader.right_field,k=a.p.treeReader.level_field,j=parseInt(b[f],10),i=parseInt(b[e],10),h=parseInt(b[k],10);
                d(this.p.data).each(function(){
                    if(parseInt(this[k],10)===h-1&&parseInt(this[f],10)<j&&parseInt(this[e],10)>i){
                        c=this;
                        return false
                        }
                    });
...
getNodeChildren:function(b){
    ...
    this.each(function(){
        ...
        if(a.grid&&a.p.treeGrid)switch(a.p.treeGridModel){
            case "nested":
                var f=a.p.treeReader.left_field,e=a.p.treeReader.right_field,k=a.p.treeReader.level_field,j=parseInt(b[f],10),i=parseInt(b[e],10),h=parseInt(b[k],10);
                d(this.p.data).each(function(){
                parseInt(this[k],10)===h+1&&parseInt(this[f],10)>j&&parseInt(this[e],10)<i&&c.push(this)
                });
            break;
            case "adjacency":
                var g=a.p.treeReader.parent_id_field,l=a.p.localReader.id;
...
getFullTreeNode:function(b){
    ...
    this.each(function(){
        ...
        if(a.grid&&a.p.treeGrid)switch(a.p.treeGridModel){
            case "nested":
                var e=a.p.treeReader.left_field,k=a.p.treeReader.right_field,j=a.p.treeReader.level_field,i=parseInt(b[e],10),h=parseInt(b[k],10),g=parseInt(b[j],10);
                d(this.p.data).each(function(){
                parseInt(this[j],10)>=g&&parseInt(this[e],10)>=i&&parseInt(this[e],10)<=h&&c.push(this)
                });
            break;
            case "adjacency":
                c.push(b);
...
expandNode:function(b){
    return this.each(function(){
        if(this.grid&&this.p.treeGrid){
            ...
            if(!b[c]){
                var a=d.jgrid.getAccessor(b,this.p.localReader.id),f=d("#"+a,this.grid.bDiv)[0],e=this.p._index[a];
...
collapseNode:function(b){
    return this.each(function(){
        if(this.grid&&this.p.treeGrid)if(b.expanded){
            ...
            d("div.treeclick",c).removeClass(this.p.treeIcons.minus+" tree-minus").addClass(this.p.treeIcons.plus+
                " tree-plus")
            }
        })
},
...
setTreeRow:function(b,c){
    ...
    this.each(function(){
        var f=this;
        if(f.grid&&f.p.treeGrid)a=d(f).jqGrid("setRowData",
            b,c)
        });
    return a
    },
        ...
        return this.each(function(){
            var d=this,c=function(e,g){
                ...
                if(xmlJsonClass.xml2json&&b.jgrid.parse){
                    ...
                    f=b.jgrid.parse(f);
                    for(var h in f)if(f.hasOwnProperty(h))k=f[h];if(g){
                        h=f.grid.datatype;
    ...
    json2xml:function(a,b){
        var g=function(d,c,j){
            ...
            for(h in d)if(d.hasOwnProperty(h))if(h.charAt(0)===
                "@")i+=" "+h.substr(1)+'="'+d[h].toString()+'"';else k=true;i+=k?">":"/>";
            if(k){
                for(h in d)if(d.hasOwnProperty(h))if(h==="#text")i+=d[h];
                    else if(h==="#cdata")i+="<![CDATA["+d[h]+"]]\>";
                    else if(h.charAt(0)!=="@")i+=g(d[h],h,j+"\t");i+=(i.charAt(i.length-1)==="\n"?j:"")+"</"+c+">"
                }
    ...
    if(a.nodeType===1){
        if(a.attributes.length){
            ...
            for(c=a.firstChild;c;c=c.nextSibling)if(c.nodeType===1)d=true;
                ...
                else if(c.nodeType===4)f+=1;if(d)if(e<2&&f<2){
                this.removeWhite(a);
                for(c=
                    a.firstChild;c;c=c.nextSibling)if(c.nodeType===3)b["#text"]=this.escape(c.nodeValue);
                    else if(c.nodeType===4)if(g.test(c.nodeValue))b[c.nodeName]=[b[c.nodeName],c.nodeValue];else b["#cdata"]=this.escape(c.nodeValue);
...
toJson:function(a,b,g){
    ...
    if(a==="[]")e+=b?":[]":"[]";
        ...
        f=[];
        for(d in a)if(a.hasOwnProperty(d))f[f.length]=this.toJson(a[d],d,g+"\t");e+=(b?":{":"{")+(f.length>1?"\n"+g+"\t"+f.join(",\n"+g+"\t")+"\n"+g:f.join(""))+
        "}"
        ...
        setColumns:function(a){
            ...
            return this.each(function(){
                ...
                if(c.grid){
                    ...
                    "function"?true:false,k=typeof a.afterShowForm==="function"?true:false,l=typeof a.afterSubmitForm==="function"?true:false,e=c.p.id,d="ColTbl_"+e,f={
                        themodal:"colmod"+e,
                        modalhead:"colhd"+e,
                    ...
                    if(b("#"+f.themodal).html()!=null){
                        ...
                        b(":input","#"+d).click(function(){
                            ...
                            if(h){
                                ...
                                a.ShrinkToFit===true&&b(c).jqGrid("setGridWidth",c.grid.width-0.001,true)
                                }
                                return this
    ...
    jQuery(n).each(function(){
        ...
        jQuery("tbody > tr",jQuery(this)).each(function(){
            ...
            jQuery("td",jQuery(this)).each(function(){
                if(e===0&&l){
                    var k=jQuery("input",jQuery(this)),m=k.val();
                    h.push(m||f.length);
            ...
            if(typeof b=="string"&&b==
                ...
                drop:function(d){
                    return a.extend({
                        ...
                        drop:function(e,h){
                            var i=a(h.draggable).attr("id");
                            i=h.draggable.parent().parent().jqGrid("getRowData",i);
                            if(!d.dropbyname){
                        ...
                        if(d.beforedrop&&a.isFunction(d.beforedrop)){
                            n=d.beforedrop.call(this,e,h,i,a("#"+c.id),a(this));
                            if(typeof n!="undefined"&&
                                n!==null&&typeof n=="object")i=n
                                }
                                if(h.helper.dropped){
                            var o;
...
gridResize:function(b){
    return this.each(function(){
        ...
        if(j.grid&&a.fn.resizable){
            ...
            b.alsoResize=b._alsoResize_?eval("("+("{'#gview_"+j.p.id+" .ui-jqgrid-bdiv':true,'"+b._alsoResize_+"':true}")+")"):a(".ui-jqgrid-bdiv",
                "#gview_"+j.p.id);
