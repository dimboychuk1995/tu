...
<html>
    ...
    <body>
        <div class="Section1">
            ...
            <div style="text-align:justify;"><p style="text-indent: 40px">АТ «Прикарпаттяобленерго», що здійснює ліцензовану  діяльність з передачі електроенергії, в особі <%if (rs.getString("type_join2").equals("1")) {%>директора філії &quot;<%= rs.getString("rem_name")%> РЕМ&quot;  <%= rs.getString("director_rod")%>, який діє на підставі  довіреності <%= rs.getString("dovirenist")%> року<%} else {%> Технічного директора АТ «Прикарпаттяобленерго» Сеника Олега Степановича, який діє на підставі довіреності № 14 від 10.05.2017 року<%}%> (Виконавець послуг), з однієї сторони, та <strong><%if (!rs.getString("customer_soc_status").equals("9")
