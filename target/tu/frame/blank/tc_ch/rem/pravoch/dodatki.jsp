...
<%  response.setHeader("Content-Disposition", "inline;filename=dodatki.doc");
    ...
    try {
        ...
        String qry = "SELECT "
                ...
                + ",isnull(convert(varchar,TC_V2.date_customer_contract_tc,104),'___.___.___') as date_customer_contract_tc"
                ...
                + ",isnull(convert(varchar(10),ch.Tc_continue_to,104),'___.___.___') as Tc_continue_to"
                + ",[rem_name] "
                + ",[Director] "
                + ",[director_rod]"
                + ",[dovirenist]"
    ...
    <body>
        <div class="Section1">
            ...
            <table border="0" cellspacing="0" cellpadding="0" align="center" width="100%">
                <tr>
                    <td width="50%" valign="top"><p align="left" class="style1"><%=rs.getString("rem_licality")%></p>
                    ...
                    <td valign="top"><p align="right" class="style1">Громадянин (ка)<br> <strong><%= rs.getString("PIP")%></strong> <br>
            ...
            <ol>
                ...
                <li>По всіх  виконаних та невиконаних зобов’язаннях по Договору <strong>№ <%= rs.getString("number")%> від <%= rs.getString("date_customer_contract_tc")%> року</strong> сторони не мають претензій одна до одної.</li>
            ...
            <table border="0" cellspacing="0" cellpadding="0" width="716">
                ...
                <tr>
                    <td width="295" valign="top"><p>
                            ...
                            Код ЄДРПОУ 00131564<br>
                            п/р 26000304757  в філії Івано-Франківське обласне управління АТ «Ощадбанк»<br>
                            Код МФО 336503 <br>
                            тел/факс: 594451 <br>
        ...
        <div class="Section1">
            ...
            <table border="0" cellspacing="0" cellpadding="0">
                ...
                <%if ((!rs.getString("power_plit").equals("0.00")) || (!rs.getString("power_boil").equals("0.00")) || (!rs.getString("power_for_electric_devices").equals("0.00"))) {%><tr>
                    <td width="347" valign="top"><span class="style1">Встановлена потужність електронагрівальних установок:</span></td>
                </tr><%}%>
            </table>
            <%}%>
            <table border="0" cellspacing="0" cellpadding="0">
                <%if (!rs.getString("power_plit").equals("0.00")) {%><tr>
                    <td width="347" valign="top">
                ...
                <%if (!rs.getString("power_boil").equals("0.00")) {%><tr>
                ...
                <%if (!rs.getString("power_for_electric_devices").equals("0.00")) {%><tr>
            ...
            <dl>
                ...
                <div align="justify" style="text-align:justify">
                    <dt>7.1. Для одержання потужності замовнику необхідно виконати:</dt>
...
<div class="Section1">
    ...
    <p align="center">&nbsp;</p>
    <p align="center"><strong>ТЕХНІЧНІ УМОВИ НЕСТАНДАРТНОГО ПРИЄДНАННЯ № <%= rs.getString("number")%></strong><br>
    </p>
    ...
    <div align="justify" style="text-align:justify">
        <dl>
            ...
            <%if (!rs.getString("power_old").equals("0")) {%><%=k++%>. Існуюча дозволена потужність згідно з договором про постачання/користування електричною енергією <strong>№ <%=rs.getString("nom_data_dog")%></strong> року <strong><%=nf.format(rs.getFloat(("power_old")))%></strong> кВт, у тому числі:</dt>
    ...
    <div class="Section1">
        ...
        <p align="right" class="style1">Додаток № 2 до додаткового правочину <strong><br>
                № П-<%= rs.getString("No_letter")%> від __.__.____ року про внесення змін до </strong><br>
            до договору про <%=rs.getString("type_join")%><br>
            до електричних мереж  від<br>
            <%= rs.getString("date_contract")%> року<br>
        ...
        <p class="style1">1 Місцезнаходження об’єкта Замовника  <%= rs.getString("type_o")%> <%= rs.getString("object_adress")%>.<br>
            2 Величина розрахункового максимального навантаження <strong><%= rs.getString("request_power")%> кВт</strong>,<br>
            у тому числі для:</p>
        <table border="0" cellspacing="0" cellpadding="0">
            <tr>
                <td width="347" valign="top">
        ...
        <dl class="style1">
            <dt>3 Напруга <%= rs.getString("joint_point").substring(16)%> кВ.</dt>
            <dt>4 Джерело електропостачання:<strong> ПС
                    <%= rs.getString("ps35110_name").substring(rs.getString("ps35110_name").indexOf(" "))%>
                    "<%= rs.getString("ps35110_name").substring(0, rs.getString("ps35110_name").indexOf(" "))%>",
                    <%=rs.getString("type_source")%>-<%= rs.getString("ps10_name")%></strong> .</dt>
            ...
            <dt>6.1 У електромережах до прогнозованої межі балансової  належності:</dt>
            <dt><% if (!rs.getString("do1").equals("")) {%>6.1.1 <%= rs.getString("do1")%><%}%></dt>
            <dt>6.2 Від межі балансової належності до електроустановок  Замовника:</dt>
            <dt>6.2.1 <%= rs.getString("do2")%></dt>
            <dt> 7. Розрахунковий облік електричної енергії: <strong><%= rs.getString("do3")%></strong>.<br> Придбання, встановлення та підключення приладів обліку електричної енергії  виконується РЕМ за рахунок абонента згідно вимог п.8 ПКЕЕН.<br>
            ...
            <dd> - Ввід  від опори (ТП, РП) повинен бути виконаний суцільним проводом без розривів до  контейнера на фасаді будинку (ввідного комутаційного пристрою п.1.5.33. ПУЕ).</dd>
            <dt>8. Влаштування  захисту від пошкоджень та обмеження дозволеної потужності: Встановити дообліковий комутаційний пристрій ( 0,4 або 0,23) кВ. Тип і номінальний струм автоматичного комутаційного пристрою визначити проектом.<br>
                У ввідному пристрої житлового будинку змонтувати пристрій захисного вимкнення (ПЗВ), його тип визначити проектом.
                <%= rs.getString("do5")%> (тип  дооблікових та післяоблікових автоматів з зазначенням допустимих величин _____________________________________________________________________________ .</dt>
            <dt>9. Змонтувати  контур заземлення на <strong>ввідній опорі</strong> з опором заземлення не більше <strong>30 </strong>Ом.
                Силова електро проводка виконується в трьох провідному виконанні. </dt>
    ...
    <div class="Section1">
        ...
        <p align="center"><strong>Додаток № 1 до додаткового правочину № П-<%= rs.getString("No_letter")%> від __.__.____року про внесення змін до<br>
                договору № <%= rs.getString("number")%> від <%= rs.getString("date_contract")%> року</strong><br />
    ...
    <div class="Section1">
        ...
        <div align="justify" style="text-align:justify">
            <dl>
                ...
                <dt>4.2. Замовник сплачує плату за приєднання,визначену в пункті 4.1 цього Договору, на поточний рахунок Виконавця послуг: п/р 26009305757 в філії Івано-Франківське обласне управління АТ «Ощадбанк», Код ЄДРПОУ 00131564, в  термін до ____ днів з дня укладання Договору.</dt>
            </dl>
        </div>
    ...
    <div class="Section1" style="font-size: 8pt;">
        ...
        <div align="justify" style="text-align:justify;"><strong>ПАТ «Прикарпаттяобленерго»</strong>, в особі технічного директора ПАТ «Прикарпаттяобленерго»&nbsp; <strong>Сеника Олега Степановича</strong>,&nbsp;який діє на підставі довіреності № 816&nbsp;від 11.08.2014&nbsp;з однієї сторони (далі - Виконавець послуг), та громадянин (ка) <strong><%=rs.getString("PIP")%></strong>, (далі – <strong>Замовник</strong>), <strong><%=rs.getString("constitutive_documents")%></strong>, ідентифікаційний код <%=rs.getString("bank_identification_number")%>, з іншої сторони (далі – Сторони), уклали цей  договір про нестандартне приєднання електроустановок Замовника до електричних мереж (далі –  Договір).<br>
        </div>
        ...
        <table border="0" cellspacing="0" cellpadding="0" class="tb8pt">
            ...
            <tr>
                <td width="328" valign="top"><p><strong>ПАТ «Прикарпаттяобленерго»</strong><br>                            
                        м. Івано-Франківськ, вул. Індустріальна, 34<br>
                        Код ЄДРПОУ 00131564<br>
                        п/р 26000304757 в філії Івано-Франківське обласне управління <br> АТ «Ощадбанк»<br>Код МФО 336503 <br>
                        тел./факс: 594451 <br>
