<%-- 
    Document   : rez_serch_json
    Created on : 30 серп 2011, 9:52:58
    Author     : AsuSV
--%>
...
<%
    ...
    try {
        ...
        pstmt = c.prepareStatement("SELECT isnull(Changestc.id,'') as id,"
                ...
                + "when type_letter=10 then 'Змінений листом'"
                ...
                + "change_date_tc,104),'') as Change_date_tc,"
                ...
                //+ "WHERE TC_V2.f_name LIKE '%имч%'");
                + "WHERE (TC_V2.f_name LIKE '%" + request.getParameter("pip") + "%')or(TC_V2.juridical LIKE '%" + request.getParameter("pip") + "%')");
        rs = pstmt.executeQuery();
        DataSource ds1 = (DataSource) ic.lookup("java:comp/env/jdbc/" + db);
        c1 = ds1.getConnection();
        pstmt1 = c1.prepareStatement("SELECT COUNT(*) AS count   FROM Changestc left join TC_V2 on Changestc.id_tc = TC_V2.id WHERE TC_V2.f_name LIKE '%" + request.getParameter("pip") + "%'");
        ...
        while (rs.next()) {
            ...
            if (rs.getString(4).equals("Правочин з додатками")) {
                type = "/pravoch/dodatki.jsp";
            } else if (rs.getString(4).equals("Продовжений листом")) {
                type = "/letter/prod.jsp";
            } else if (rs.getString(4).equals("Продовжений по правочину")) {
                type = "/pravoch/prod.jsp";
            } else if (rs.getString(4).equals("Розірваний листом")) {
                type = "/letter/roz.jsp";
            } else if (rs.getString(4).equals("Розірваний по правочину")) {
                type = "/pravoch/roz.jsp";
            } else if (rs.getString(4).equals("Змінений листом")) {
                type = "/letter/zmin.jsp";
            } else if (rs.getString(4).equals("Змінений по правочину")) {
                type = "/pravoch/zmin.jsp";
            } else if (rs.getString(4).equals("Неукладений")) {
                type = "/letter/neukl.jsp";
            } else if (rs.getString(4).equals("Неукладений")) {
                type = "/pravoch/neukl.jsp";
            } else if (rs.getString(4).equals("Лист відмова")) {
                type = "/letter/vidmova.jsp";
            } else if (rs.getString(4).equals("Лист попередження")) {
                type = "/letter/popereg.jsp";
            } else if (rs.getString(4).equals("Неукладений(приєднання)")) {
                type = "/letter/neukl_join.jsp";
            } else if (rs.getString(4).equals("Розірваний по правочину(приєднання)")) {
                type = "/pravoch/roz_join.jsp";
            } else if (rs.getString(4).equals("Лист про продовження строку приєднання")) {
                type = "/letter/extension_of_term.jsp";
            } else if (rs.getString(4).equals("Лист-звернення до власника земельної ділянки")) {
                type = "/letter/appeals_to_owner.jsp";
            } else if (rs.getString(4).equals("Додаткова угода про оплату нестандартного приєднання")) {
                type = "/letter/dod_deal.jsp";
            } else if (rs.getString(4).equals("Лист-відмова у стандартному приєднанні")) {
