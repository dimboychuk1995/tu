...
<html>
    ...
    <body>     
        ...
        <div align="justify" style="text-align:justify;"><strong>АТ «Прикарпаттяобленерго»</strong>, в особі технічного директора <strong>Сеника Олега Степановича</strong>, який діє на підставі довіреності № 14 від 10.05.2017 з однієї сторони, та <% if (rs.getString("customer_type").equals("0")) {%>громадянин(ка) <strong><%=rs.getString("PIP")%></strong>, (далі – Замовник), <strong><%=rs.getString("constitutive_documents")%></strong>, ідентифікаційний код <%=rs.getString("bank_identification_number")%>,<%} else {
