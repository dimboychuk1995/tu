...
<%  response.setHeader("Content-Disposition","inline;filename=calc_ns_join_1.doc") ;
    ...
    try {
        c = ds.getConnection();
           ...
           + "        ,@price_conn NUMERIC (10,2) = 412.30 \n"
           + " SET @connection_price =(SELECT CASE WHEN (isnull(tv.request_power,0.00))<=15 THEN 1029.93 \n"
           + "					  WHEN (isnull(tv.request_power,0))>15 AND (isnull(tv.request_power,0))<=150 THEN 1056.52 \n"
           + "					  WHEN (isnull(tv.request_power,0))>150 AND (isnull(tv.request_power,0))<=500 THEN 1719.30 \n"
           + "					  WHEN (isnull(tv.request_power,0))>500 AND (isnull(tv.request_power,0))<=10000 THEN 1840.68 \n"
