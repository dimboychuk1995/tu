...
this._xmlParser = function() {
    ...
    for (var q=0;q<root.childNodes.length;q++)if (root.childNodes[q].tagName == "item")main_self._addItemToStorage(root.childNodes[q]);main_self.callEvent("onXLE", []);
    main_self._doOnLoad();
    this.destructor()
    };
...
(function(){
    ...
    var functor=function(name,res){
        return function(itemId,a,b){
            ...
            if (this.objPull[itemId][name] != null)return this.objPull[itemId][name].call(this.objPull[itemId],a,b)
            else 
                return res
                }
            };
...
this.getListOptionPosition = function(id) {
    ...
    for (var q=0;q<this.polygon.childNodes.length;q++)if (this.polygon.childNodes[q] == that.objPull[id].obj)pos = q+1;return pos
    };
...
this.getListOptionText = function(id) {
    ...
    if (that.objPull[id].obj.className == "btn_sep")return;
    var obj = that.objPull[id].obj;
    for (var q=0;q<obj.childNodes.length;q++)if (String(obj.childNodes[q].tagName).toLowerCase() == "span") text = obj.childNodes[q].innerHTML;return text
    };
...
this.getAllListOptions = function() {
    ...
    for (var a in this._listOptions)listData[listData.length] = a;return listData
    };
...
this.getListOptionSelected = function() {
    var id = null;
    for (var a in this._listOptions)if (this._listOptions[a]._selected == true)id = a;return id
    };
...
dhtmlXToolbarObject.prototype._buttonInputObject = function(that, id, data) {
    ...
    this.obj.title = (data.getAttribute("title")!=null?data.getAttribute("title"):"");
    this.obj.innerHTML = "<input class='inp' type='text' style='-moz-user-select:text;width:"+this.obj.w+"px;'"+(data.getAttribute("value")!=null?"' value='"+data.getAttribute("value")+"'":"")+">";
    var th = that;
    var self = this;
